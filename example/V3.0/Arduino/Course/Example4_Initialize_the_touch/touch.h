/*---------------------------------------------------------------
 * Touch controller selection and calibration
 * Configure the GT911 device installed on this display board.
 *--------------------------------------------------------------*/

// The GT911 communicates through I2C; unused interrupt and reset pins use -1.
#define TOUCH_GT911
...
// Fixed panel dimensions form the destination range for coordinate mapping.
...
#include <SPI.h>
XPT2046_Touchscreen ts(TOPT2046_CS, TOUCH_XPT2046_INT);
...
void touch(TPoint p, TEvent e)
{
  ...
  touch_last_x = map(p.y, TOUCH_MAP_X1, TOUCH_MAP_X2, 0, LCD_WIDTH);
  touch_last_y = map(p.x, TOUCH_MAP_Y1, TOUCH_MAP_Y2, 0, LCD_HEIGHT);
#else
  touch_last_x = map(p.x, TOUCH_MAP_X1, TOUCH_MAP_X2, 0, LCD_WIDTH);
  touch_last_y = map(p.y, TOUCH_MAP_Y1, TOUCH_MAP_Y2, 0, LCD_HEIGHT);
#endif
  switch (e)
  {
  case TEvent::Tap:
 ...
 * @note Called at the start of each input polling cycle.
 */
 ...
 * Hide controller-specific polling details from the application.
 *--------------------------------------------------------------*/
...
bool touch_touched()
{
  ...
  {
#if defined(TOUCH_SWAP_XY)
    touch_last_x = map(ts.points[0].y, TOUCH_MAP_X1, TOUCH_MAP_X2, 0, LCD_WIDTH - 1);
    touch_last_y = map(ts.points[0].x, TOUCH_MAP_Y1, TOUCH_MAP_Y2, 0, LCD_HEIGHT - 1);
#else
    touch_last_x = map(ts.points[0].x, TOUCH_MAP_X1, TOUCH_MAP_X2, 0, LCD_WIDTH - 1);
    touch_last_y = map(ts.points[0].y, TOUCH_MAP_Y1, TOUCH_MAP_Y2, 0, LCD_HEIGHT - 1);
#endif
    return true;
  }
  else
...
#if defined(TOUCH_SWAP_XY)
    touch_last_x = map(p.y, TOUCH_MAP_X1, TOUCH_MAP_X2, 0, LCD_WIDTH - 1);
    touch_last_y = map(p.x, TOUCH_MAP_Y1, TOUCH_MAP_Y2, 0, LCD_HEIGHT - 1);
#else
    touch_last_x = map(p.x, TOUCH_MAP_X1, TOUCH_MAP_X2, 0, LCD_WIDTH - 1);
    touch_last_y = map(p.y, TOUCH_MAP_Y1, TOUCH_MAP_Y2, 0, LCD_HEIGHT - 1);
#endif
    return true;
  }
  else
